...
create table subdivisions (holiday_id uuid not null, holiday_code varchar(255) not null, holiday_iso_code varchar(255) not null, holiday_name varchar(255) not null, holiday_short_name varchar(255) not null);
create table time_entries (date date not null, pause_time numeric(21,0) not null, rate numeric(38,2) not null, total numeric(38,2) not null, total_internal numeric(38,2) not null, total_time numeric(21,0) not null, created_date timestamp(6) with time zone not null, end_time timestamp(6) with time zone, last_modified_date timestamp(6) with time zone not null, start_time timestamp(6) with time zone, version bigint not null, created_by_id uuid, employee_id uuid not null, id uuid not null, last_modified_by_id uuid, project_id uuid not null, description varchar(1000) not null, created_by_username varchar(255) not null, last_modified_by_username varchar(255) not null, no varchar(255) unique, primary key (id));
create table user_roles (user_id uuid not null, role varchar(255) check (role in ('EXTERNAL','USER','MANAGER','ADMIN','SUPERUSER')));
create table users (is_enabled boolean not null, is_locked boolean not null, created_date timestamp(6) with time zone not null, last_modified_date timestamp(6) with time zone not null, version bigint not null, created_by_id uuid, id uuid not null, last_modified_by_id uuid, username varchar(20) not null unique, created_by_username varchar(255) not null, email varchar(255) not null, first_name varchar(255) not null, last_modified_by_username varchar(255) not null, last_name varchar(255) not null, password varchar(255) not null, primary key (id));
alter table if exists employees add constraint FK69x3vjuy1t5p18a5llb8h2fjx foreign key (user_id) references users;
alter table if exists project_assignments add constraint FKrci4e53hdoxpmo9pgy9dta7e9 foreign key (employee_id) references employees;
alter table if exists project_assignments add constraint FKbwds7sls0j9asmsm2do8cn8co foreign key (project_id) references projects;
alter table if exists subdivisions add constraint FKc2sae6upd4hrowl9okku0xuy foreign key (holiday_id) references holidays;
alter table if exists time_entries add constraint FK7ts9hn6i2fjkskn5j79ja6p0v foreign key (employee_id) references employees;
alter table if exists time_entries add constraint FK7p5j2i2xr2q8moop1x2g6wol foreign key (project_id) references projects;
