...
import { useDeleteHolidaysFromYear } from "@/api/rest/generated/query/holiday-import/holiday-import";
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuGroup,
...
export default function HolidayDataTable( { holidays, year }: Readonly<HolidayDataTableProps> ) {
    ...
    const sortedHolidays = useMemo( () => holidays
            .toSorted( ( a, b ) => isAfter( a.startDate!, b.startDate! ) ? 1 : -1 )
        , [ holidays ]
    );
    ...
    const handleDeleteHolidays = async ( year: string ) => {
        mutate(
            ...
            {
                onSuccess: () => {
                    invalidateAllQueries( queryClient, getImportHolidaysInvalidations( year ) ).then( () => {
